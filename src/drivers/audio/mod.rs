//! Audio — Intel HD Audio (HDA) controller driver
//!
//! Intel HDA (ICH6+) is the de facto standard for PC audio.
//! PCI vendor ID = 0x8086, device IDs vary (0x2668, 0x27D8, 0x284B, etc.)
//!
//! # HDA Architecture
//! - **Controller** → **Codec** (via HDA Link)
//! - **Codec** → widgets (DAC, ADC, mixer, pin complexes)
//! - **Streams** use BDL (Buffer Descriptor List) with DMA rings
//!
//! # Current status
//! - ✓ HDA controller detection (PCI scan)
//! - ✓ GCTL reset + CORB/RIRB initialisation
...
//!
//! For demonstration, boot audio notification works. Real‑time audio requires DMA IRQ.
//! Full audio (mixer, multi‑stream, codec enumeration) is ~2000 lines.

use spin::Mutex;
use crate::pci::PciDevice;

// -----------------------------------------------------------------------------
// HDA register offsets (global)
// -----------------------------------------------------------------------------
...
// -----------------------------------------------------------------------------
// HDA Stream Descriptor offsets (relative to stream base, e.g., SD0_OFFSET = 0x80)
// -----------------------------------------------------------------------------

const SD0_OFFSET: u32 = 0x80;   // Stream Descriptor 0 (first output stream)
...
const SD_CTL_RESET: u8 = 0x01;  // Stream Reset bit

// -----------------------------------------------------------------------------
// CORB / RIRB offsets (Command Output Ring Buffer / Response Input Ring Buffer)
// -----------------------------------------------------------------------------

const CORB_OFFSET: u32 = 0x40;  // Command Output Ring Buffer base
const RIRB_OFFSET: u32 = 0x50;  // Response Input Ring Buffer base
...
pub struct HdaController {
    ...
    pub mmio_base: usize,
    /// Bitmask of detected codecs (each bit corresponds to a codec)
    pub codec_mask: u16,
}
...
/// Try to initialise the Intel HDA controller from a PCI device.
...
pub fn try_init(dev: &PciDevice) -> bool {
    ...
    unsafe {
        // Reset controller: write 0 to GCTL.CRST (active low), then 1
        ...
        // Clear by writing back the same value (spec: write 1 to bits to clear)
        write16(mmio, HDA_STATSTS, statsts);
...
/// Returns `true` if an HDA controller was successfully initialised.
pub fn is_available() -> bool {
    HDA.lock().is_some()
}

/// Returns the codec presence mask (bits 0‑15).
pub fn codec_mask() -> u16 {
    HDA.lock().as_ref().map(|h| h.codec_mask).unwrap_or(0)
}

// -----------------------------------------------------------------------------
// Stream DMA — Buffer Descriptor List (BDL)
// -----------------------------------------------------------------------------

/// Set up output stream 0 with a BDL for PCM playback.
/// The stream format is hardcoded as 0x4011 (16‑bit stereo, 44.1 kHz).
///
/// # Arguments
/// * `pcm_buf` – Raw PCM data bytes (will be copied into a DMA buffer).
///
/// # Returns
...
pub fn setup_output_stream(pcm_buf: &[u8]) -> bool {
    ...
    unsafe {
        ...
        for i in 0..BDL_ENTRIES {
            ...
            entry.flags = if i == BDL_ENTRIES - 1 { 1 } else { 0 }; // IOC on last entry
        }
...
/// Play PCM audio data (16‑bit signed samples).
/// Converts `i16` samples to raw bytes and starts playback.
///
/// # Arguments
/// * `data` – Slice of 16‑bit PCM samples.
///
/// # Returns
...
pub fn play_pcm(data: &[i16]) -> bool {
    ...
    let pcm_bytes: alloc::vec::Vec<u8> = data
        .iter()
...
/// Sends an AMP gain verb to codec 0, node 0 (simplified).
///
/// # Arguments
/// * `percent` – Volume percentage (0 = mute, 100 = max).
pub fn set_volume(percent: u8) {
    ...
    // Verb format: (codec << 28) | (node << 20) | (verb_id << 8) | payload.
...
// -----------------------------------------------------------------------------
// Codec enumeration (debug / diagnostic)
// -----------------------------------------------------------------------------

/// Walk all detected codecs and enumerate their widgets.
/// Prints information to the serial console.
...
// -----------------------------------------------------------------------------
// HDA verb send/receive (CORB/RIRB)
// -----------------------------------------------------------------------------

/// Send an HDA verb to a codec and wait for the response.
///
/// # Arguments
/// * `base` – MMIO base address of the HDA controller.
/// * `verb` – 32‑bit verb as described in the HDA specification.
///
/// # Returns
/// The 32‑bit response from the codec, or 0 on timeout.
pub fn codec_verb(base: usize, verb: u32) -> u32 {
    unsafe {
        // Write the verb to CORB (Command Output Ring Buffer).
        let corb_wp = read32(base, CORB_WP) & 0xFF;
        let new_wp = (corb_wp + 1) & 0xFF;
        let corb_base = read32(base, 0x44) as usize; // CORB base address (low)
        if corb_base != 0 {
            let corb = (PHYS_OFFSET + corb_base) as *mut u32;
            corb.add(new_wp as usize).write_volatile(verb);
            write8(base, CORB_WP, new_wp as u8);
        }
        // Wait for RIRB (Response Input Ring Buffer) to produce a response.
        ...
        while crate::arch::x86_64::timer::uptime_ms() < deadline {
            ...
            if rirb_wp != corb_wp {
                let rirb_base = read32(base, 0x54) as usize; // RIRB base address (low)
                if rirb_base != 0 {
                    let rirb = (PHYS_OFFSET + rirb_base) as *const u64;
...
unsafe fn read8(base: usize, offset: u32) -> u8 {
    core::ptr::read_volatile((base + offset as usize) as *const u8)
}

#[inline(always)]
unsafe fn read16(base: usize, offset: u32) -> u16 {
    core::ptr::read_volatile((base + offset as usize) as *const u16)
}
...
#[inline(always)]
unsafe fn write8(base: usize, offset: u32, value: u8) {
    core::ptr::write_volatile((base + offset as usize) as *mut u8, value);
}

#[inline(always)]
unsafe fn write16(base: usize, offset: u32, value: u16) {
    core::ptr::write_volatile((base + offset as usize) as *mut u16, value);
}
