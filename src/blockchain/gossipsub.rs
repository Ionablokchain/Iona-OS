...
//! │                          Gossipsub Module                              │
//! ├─────────────┬──────────────┬───────────────┬──────────────────────────┤
//! │   config    │    error     │    message    │        peer              │
//! │ (GossipCfg) │ (GossipError)│ (MsgType, Msg)│ (Peer, state)            │
//! ├─────────────┼──────────────┼───────────────┼──────────────────────────┤
//! │     node    │   discovery  │    metrics    │        manager           │
//! │ (GossipNode)│ (PeerDiscovery)│ (metrics)   │ (GossipManager)          │
//! └─────────────────────────────────────────────────────────────────────────┘
//! ```
//!
//! # Example
//!
//! ```rust,ignore
//! use iona::p2p::gossipsub::{GossipManager, GossipConfig};
//!
//! let config = GossipConfig::default();
//! let mut manager = GossipManager::new(port, config).unwrap();
//! manager.subscribe("iona/blocks");
//! manager.publish("iona/blocks", b"hello".to_vec()).unwrap();
//! ```
...
pub mod config {
    ...
    impl GossipConfig {
        pub fn validate(&self) -> Result<(), &'static str> {
            if self.d == 0 || self.d_low == 0 || self.d_high == 0 {
                return Err("mesh degrees must be > 0");
            }
            if self.d_low > self.d || self.d > self.d_high {
                return Err("invalid mesh degree bounds: d_low <= d <= d_high");
            }
            if self.history == 0 {
                return Err("history must be > 0");
            }
            if self.max_msg_size < 1024 {
                return Err("max_msg_size must be at least 1024");
            }
            if self.heartbeat_ms == 0 {
                return Err("heartbeat_ms must be > 0");
            }
            Ok(())
        }
...
pub mod message {
    ...
    use super::error::{GossipError, GossipResult};
    use alloc::{
        string::{String, ToString},
        vec::Vec,
    };
    ...
    impl fmt::Display for MsgType {
        fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
            match self {
                ...
                Self::IHave => write!(f, "IHave"),
                Self::IWant => write!(f, "IWant"),
    ...
    impl GossipMessage {
        ...
        pub fn new_publish(topic: &str, data: Vec<u8>) -> Self {
            ...
            Self {
                ...
                data,
                ttl: super::constants::DEFAULT_TTL,
            }
        }
...
pub mod peer {
    ...
    impl Peer {
        ...
        /// Check if the peer is subscribed to a topic.
        pub fn is_subscribed(&self, topic: &str) -> bool {
            self.topics.contains(topic)
        }

        /// Check if the peer is in the mesh for a topic.
        pub fn in_mesh(&self, topic: &str) -> bool {
            self.mesh.contains(topic)
        }
    }
    ...
    impl fmt::Display for Peer {
        fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
            write!(
                ...
                self.port, self.score, self.topics
            )
        }
    }
}
...
pub mod metrics {
    ...
    pub struct GossipMetrics {
        ...
        pub graft_sent: AtomicU64,
        pub graft_received: AtomicU64,
        pub prune_sent: AtomicU64,
        pub prune_received: AtomicU64,
    }
    ...
    impl GossipMetrics {
        ...
        pub fn set_peers(&self, count: usize) {
            self.peers_connected.store(count as u64, Ordering::Relaxed);
        }
        pub fn inc_evicted(&self) {
            self.peers_evicted.fetch_add(1, Ordering::Relaxed);
        }
        pub fn inc_graft_sent(&self) {
            self.graft_sent.fetch_add(1, Ordering::Relaxed);
        }
        pub fn inc_graft_received(&self) {
            self.graft_received.fetch_add(1, Ordering::Relaxed);
        }
        pub fn inc_prune_sent(&self) {
            self.prune_sent.fetch_add(1, Ordering::Relaxed);
        }
        pub fn inc_prune_received(&self) {
            self.prune_received.fetch_add(1, Ordering::Relaxed);
        }
        ...
        pub fn snapshot(&self) -> GossipMetricsSnapshot {
            GossipMetricsSnapshot {
                ...
                graft_sent: self.graft_sent.load(Ordering::Relaxed),
                graft_received: self.graft_received.load(Ordering::Relaxed),
                prune_sent: self.prune_sent.load(Ordering::Relaxed),
                prune_received: self.prune_received.load(Ordering::Relaxed),
            }
        }
    }
    ...
    pub struct GossipMetricsSnapshot {
        ...
        pub prune_sent: u64,
        pub prune_received: u64,
    }
}

pub mod node {
    //! Main Gossipsub node implementation.
    use super::{
        config::GossipConfig,
        error::{GossipError, GossipResult},
        message::{GossipMessage, MsgType},
        peer::Peer,
    ...
    pub struct GossipNode {
        ...
        pub mesh: BTreeMap<String, BTreeSet<u64>>,
        pub seen_msgs: VecDeque<u32>,
        pub inbox: VecDeque<(String, Vec<u8>)>,
    ...
    impl GossipNode {
        ...
        pub fn add_peer(&mut self, ip: [u8; 4], port: u16) {
            ...
            if !self.peers.contains_key(&k) {
                ...
                self.metrics.set_peers(self.peers.len());
            }
        }
        ...
        pub fn metrics_snapshot(&self) -> super::metrics::GossipMetricsSnapshot {
        ...
        fn handle_graft(&mut self, msg: GossipMessage, from: u64) -> GossipResult<()> {
            self.metrics.inc_graft_received();
    ...
    impl fmt::Debug for GossipNode {
        fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
            f.debug_struct("GossipNode")
                ...
                .field("mesh_topics", &self.mesh.len())
...
pub mod discovery {
    ...
    impl PeerDiscovery {
        ...
        pub fn node_mut(&mut self) -> &mut GossipNode {
            &mut self.node
        }

        /// Get the underlying node (immutable).
        pub fn node(&self) -> &GossipNode {
            &self.node
        }
    }

    impl fmt::Debug for PeerDiscovery {
        fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
            f.debug_struct("PeerDiscovery")
                .field("node", &self.node)
                .field("states", &self.states.len())
                .field("reconnect_at", &self.reconnect_at.len())
...
pub mod manager {
    //! Centralised Gossipsub manager.
    use super::{
        config::GossipConfig,
        error::{GossipError, GossipResult},
        node::GossipNode,
        discovery::PeerDiscovery,
        metrics::GossipMetricsSnapshot,
    };
    use alloc::vec::Vec;
    use tracing::info;

    /// Centralised manager for Gossipsub.
    ...
    impl GossipManager {
        ...
        pub fn new(port: u16, config: GossipConfig) -> GossipResult<Self> {
            config.validate().map_err(|e| GossipError::Config(e.to_string()))?;
            info!(port, "gossip manager created");
            Ok(Self {
                discovery: PeerDiscovery::new(port, config.clone()),
                config,
            })
        }
        ...
        /// Subscribe to a topic.
        pub fn subscribe(&mut self, topic: &str) {
            self.discovery.node.subscribe(topic);
        }

        /// Unsubscribe from a topic.
        pub fn unsubscribe(&mut self, topic: &str) {
            self.discovery.node.unsubscribe(topic);
        }

        /// Publish a message.
        pub fn publish(&mut self, topic: &str, data: Vec<u8>) -> GossipResult<u32> {
            self.discovery.node.publish(topic, data)
        }

        /// Poll for incoming messages.
        pub fn poll(&mut self) -> GossipResult<bool> {
            self.discovery.node.poll()
        }

        /// Receive one message from the inbox.
        pub fn recv(&mut self) -> Option<(String, Vec<u8>)> {
            self.discovery.node.recv()
        }

        /// Run the heartbeat and discovery tick.
        pub fn tick(&mut self) {
            self.discovery.tick();
        }
        ...
        /// Add a known peer.
        pub fn add_peer(&mut self, ip: [u8; 4], port: u16) {
            self.discovery.node.add_peer(ip, port);
        }

        /// Get the number of connected peers.
        pub fn connected_count(&self) -> usize {
            self.discovery.connected_count()
        }

        /// Get metrics snapshot.
        pub fn metrics(&self) -> GossipMetricsSnapshot {
            self.discovery.node.metrics_snapshot()
        }

        /// Get the mesh size for a topic.
        pub fn mesh_size(&self, topic: &str) -> usize {
            self.discovery.node.mesh_size(topic)
        }

        /// Get the peer count.
        pub fn peer_count(&self) -> usize {
            self.discovery.node.peer_count()
        }

        /// Get the underlying node (for advanced use).
        pub fn node_mut(&mut self) -> &mut GossipNode {
            self.discovery.node_mut()
        }
...
pub use message::{GossipMessage, MsgType};
pub use peer::Peer;
pub use node::GossipNode;
pub use discovery::{PeerDiscovery, PeerState};
pub use metrics::{GossipMetrics, GossipMetricsSnapshot};
...
mod tests {
    ...
    fn test_peer_addr_string() {
        let p = Peer::new([192, 168, 1, 1], 8080);
        assert_eq!(p.addr_string(), "192.168.1.1:8080");
    }

    #[test]
    fn test_msg_type_display() {
        assert_eq!(format!("{}", MsgType::Publish), "Publish");
        assert_eq!(format!("{}", MsgType::IHave), "IHave");
    }

    #[test]
    fn test_peer_methods() {
        let mut p = Peer::new([10, 0, 0, 1], 1234);
        assert!(p.connected);
        p.topics.insert("test".into());
        assert!(p.is_subscribed("test"));
        assert!(!p.is_subscribed("other"));
        p.mesh.insert("test".into());
        assert!(p.in_mesh("test"));
    }
}
