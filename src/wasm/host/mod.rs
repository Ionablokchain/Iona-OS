//! WASM host functions complete — tot ce expune kernelul modulelor WASM
//!
//! Implementare completă pentru portarea iona-node pe IONA OS:
//!   log_write, storage_get/set, emit_event (existente)
//!   + tcp_connect/send/recv/close (noi)
//!   + udp_bind/sendto/recvfrom (noi)
//!   + ipc_send/recv (noi)
//!   + get_uptime, get_chain_rpc (noi)
...
fn register_storage(linker: &mut Linker<WasmState>) -> Result<(), wasmi::Error> {
    linker.func_wrap("env", "storage_get", |mut caller: Caller<WasmState>, kp: i32, kl: i32, vp: i32, vc: i32| -> i32 {
        let mem = match caller.get_export("memory").and_then(|e| e.into_memory()) { Some(m) => m, None => return -1 };
...
fn register_network(linker: &mut Linker<WasmState>) -> Result<(), wasmi::Error> {
    ...
    linker.func_wrap("env", "tcp_send", |caller: Caller<WasmState>, fd: i64, ptr: i32, len: i32| -> i32 {
        let mem  = match caller.get_export("memory").and_then(|e| e.into_memory()) { Some(m) => m, None => return 0 };
    ...
    linker.func_wrap("env", "tcp_recv", |mut caller: Caller<WasmState>, fd: i64, ptr: i32, cap: i32| -> i32 {
        ...
        if n > 0 { mem.data_mut(&mut caller)[ptr as usize..ptr as usize+n].copy_from_slice(&tmp[..n]); }
        n as i32
    })?;
...
fn register_fs(linker: &mut Linker<WasmState>) -> Result<(), wasmi::Error> {
    // fs_read(path_ptr, path_len, buf_ptr, buf_cap) -> i32 (bytes read, or -1)
    ...
    // fs_exists(path_ptr, path_len) -> i32 (1=exists, 0=not)
    ...
    // fs_delete(path_ptr, path_len) -> i32 (1=ok, 0=fail)
